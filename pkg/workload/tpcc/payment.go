...
var _ tpccTx = &paymentRead{}

func createPaymentRead(
	...
	if err := p.sr.Init(ctx, "payment-read", mcp); err != nil {
		return nil, err
	}
...
var _ tpccTx = &paymentWrite{}

func createPaymentWrite(
	ctx context.Context, config *tpcc, mcp *workload.MultiConnPool,
) (tpccTx, error) {
	p := &paymentWrite{
		config: config,
	...
	if err := p.sr.Init(ctx, "payment-write", mcp); err != nil {
		return nil, err
	}
...
func (p *paymentRead) run(ctx context.Context, wID int) (interface{}, error) {
	atomic.AddUint64(&p.config.auditor.paymentTransactions, 1)
	...
	// 2.5.1.2: The customer is randomly selected by last name
	d.cLast = string(p.config.randCLast(rng, &p.a))
	atomic.AddUint64(&p.config.auditor.paymentsByLastName, 1)
...
func (p *paymentWrite) run(ctx context.Context, wID int) (interface{}, error) {
	atomic.AddUint64(&p.config.auditor.paymentTransactions, 1)
	...
	if err := crdbpgx.ExecuteTx(
		...
		func(tx pgx.Tx) error {
			...
			if err := p.updateWithPayment.QueryRowTx(
				...
				return errors.Wrap(err, "select by customer id fail")
			}
